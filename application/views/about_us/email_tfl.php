...
#{
	#sleep(10);	
	$sql = <<<SQL
    		#SELECT * FROM teles_bin.for_email_member_off WHERE type = 2
	    	#SELECT *
			...
			SELECT *
			FROM teles_bin.email_gadget2013
			WHERE status = 0
			LIMIT 50
...
SQL;
	...
	foreach($query->result() as $row)
	{
		...
		$data["body"] = '<img src="cid:TELES_Comics.jpg" />';
		$this->email->attach($path.'/assets/files/Teles_Comics.jpg');
		$this->email->attach($path.'/assets/files/TFL_computation.xls');
		$emailbody = $this->load->view('mail_view.php',$data,true);
		$this->email->subject('Tuition Fee Loan (TFL)');
		$this->email->message($emailbody);    
			
		if ( ! $this->email->send())
		{
			$this->db->where('id',$row->id);
			$this->db->update('teles_bin.email_3g',array('status'=> 2));
			
		    echo $ctr++.'. The email error '.$row->email_ads .' - '.date('h:i:s').'<br>';
		}else{
			
			$this->db->where('id',$row->id);
			$this->db->update('teles_bin.email_3g',array('status'=> 1));
