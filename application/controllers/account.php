...
class Account extends CI_Controller {
	...
	*/
	...
	function process_online_po()
	{
		...
		foreach($query->result() as $row)
		{
			...
			if($row->prod_id == 'O-DS01')
			{
				...
				$this->save_inventory_trans($sales_id);
			}
	...
	function save_inventory_trans($sales_id)
	{
				...
				if(!empty($answer))
				{
					...
					if($email == $db_email_ads)
					{
						if($answer == $db_answer)
						{
							...
							echo "<script>window.location.replace('http://119.93.95.162/For_Evaluation/index.php/Welcome/forgot_pwd/$member_id')</script>";
    ...
    function submit_inquiry()
    {
    	...
    	else
    	{
	    	...
	    	else
	    	{
	    		...
	    		*/
	    		...
	    		echo "<script>window.location.replace('http://119.93.95.162/For_Evaluation/index.php/Welcome/email_inquiries/$member_id/$title/$msg')</script>";
	   ...
	   //  		$query = $this->db->query($sql);
	    ...
	    if($title == '1')
		{
			...
			$for_email = "armi.geneta@telescoop.com.ph, raymond.mendoza@telescoop.com.ph, eirleen.cruz@telescoop.com.ph";
		}
		else if($title == '2')
		{
			$title = "Savings Deposit / Withdrawal / Balance";
			$for_email = "bhelle.deguzman@telescoop.com.ph, marisa.bravo@telescoop.com.ph, janica.hermosora@telescoop.com.ph";
		}
		else if($title == '3')
		{
			$title = "Direct Selling Loans / Promo Loans / Pricelist";
			$for_email = "imee.armada@telescoop.com.ph, gigi.deguzman@telescoop.com.ph, nori.balane@telescoop.com.ph, kaira.alquuigue.@telescoop.com.ph";
		}
		else if($title == '4')
		{
			$title = "Application for New / Termination of Membership";
			$for_email = "loraine.tatad@telescoop.com.ph, michelle.balabbo@telescoop.com.ph, divine.buenvenida@telescoop.com.ph";
		}
		else if($title == '5')
		{
			$title = "Others";
			$for_email = "gie.armada@telescoop.com.ph, guinmar.liamzon@telescoop.com.ph, anthony.manzano@telescoop.com.ph";
		}
		else if($title == '6')
		{
    ...
    //                 );
			...
			#	$send_na = $this->mailguinmar->send($config);
				#$send_na1 = $this->mailguinmar->send($config1);
	...
	function for_evaluation()
	{
		...
		*/
		...
		echo "<script>window.location.replace('http://119.93.95.162/For_Evaluation/index.php/Welcome/')</script>";
	}
		...
		if(isset($_POST['login']))
		{
			...
			#echo $password;
				...
				$sql = 'SELECT *
						...
						LEFT JOIN mem_members A USING(member_id)
						WHERE username = "'.$username.'"
			...
			$last_log = $query->row('last_login');//guinmar doing for validation as per BMP
			$member_id = $query->row('member_id');//guinmar doing for validation as per BMP
			...
			if($query->num_rows() == 1 AND $status == 1)
			{
				...
				{
					//guinmar doing for validation as per BMP
					if(empty($last_log))
					{
						$newdata = array('member_id' => $member_id);
						$this->session->set_userdata($newdata);

						$val_att = "SELECT * FROM telescoop_web.val_attempt WHERE member_id = $member_id";
						$val_atts = $this->db->query($val_att);
						$val_attempt = $val_atts->row('val_attempt');
						if($val_atts->num_rows() == 0)
						{
						$val_attempt = "INSERT INTO telescoop_web.val_attempt (id,member_id,val_attempt)
										VALUES ('',$member_id,'0')";

						$this->db->query($val_attempt);
						}
						...
						if($val_attempt >= 3)
						{
							...
							echo "
									...
									<script>alert('Your account is Locked! Call our MIS @ 8-890-04-31 to unlock account!')
					...
					//end guinmar doing. uncomment below to activate ung dati and comment this.
	...
	// GUINAMR DOINGS validation as per bmp
	function validation_bmp($send ='')
	{
		$member_id1 = $this->session->userdata('member_id');
		if(empty($member_id1))
		{
			$member_id1 = $send;
		}

		$val_att = "SELECT * FROM telescoop_web.val_attempt WHERE member_id = $member_id1";
		$val_atts = $this->db->query($val_att);
		$val_attempt = $val_atts->row('val_attempt');
		...
		if(!empty($send))
		{
			$sql_send = "SELECT * FROM mem_members WHERE member_id = '$send'";
			$query_send = $this->db->query($sql_send);

			$newdata = array(
		                   'is_login'  => TRUE,
		                   'email'     => $query_send->row('mem_email'),
		                    'member_id'     => $query_send->row('member_id'),
		                   'username' => $query_send->row('username'),
		                   'name' =>  $query_send->row('mem_fname').' '.$query_send->row('mem_lname')
		               );
		...
		else
		{
			...
			$emp_no = $this->input->post('emp_num');
			$member_id = $this->input->post('member_id');
			$var_bday = $this->input->post('birthday');
			$bday = date('Y-m-d',strtotime($var_bday));
			$this->form_validation->set_rules('emp_num', 'Employee No', 'callback_emp_num_check|required|alpha_numeric');
			$this->form_validation->set_rules('birthday', 'Birth date', 'callback_birthday_check|required');


			if($val_atts->row('val_attempt') >= 3)
			{

				echo "<script>window.location.replace('https://www.telescoop.com.ph/For_Evaluation/index.php/Welcome/failed_attempt/$member_id')</script>";
				// echo "
			...
			else
			{
				...
				if ($this->form_validation->run() == FALSE)
				{
					...
					$update_val = "UPDATE telescoop_web.val_attempt SET val_attempt = $val_attempt WHERE member_id = $member_id";
					$this->db->query($update_val);
				...
				else
				{
					...
					$sql = "SELECT * FROM mem_members WHERE member_id = '$member_id' AND (mem_emp_id = '$emp_no' OR mem_emp_id2 = '$emp_no') AND mem_bday = '$bday'";
					$query = $this->db->query($sql);
					...
					if($query->num_rows() == 1)
					{
						...
						$newdata = array(
							...
							echo "<script>window.location.replace('https://www.telescoop.com.ph/For_Evaluation/index.php/Welcome/send_congrats/$member_id')</script>";
	...
	// GUINMAR DOINGS TO COMPENSATE BLISS OF THE NEW FACE OF THE WEBSITE
	function profile()
	{
		$member_id = $this->session->userdata('member_id');
	...
	{
		$member_id = $this->session->userdata('member_id');
		$member_id1 = $this->input->post('member_id');
		if(!empty($member_id1))
		{
		...
		if(!$this->session->userdata('is_login'))
		{
			...
			<script>
				...
				alert('Session expired. Please login to continue.12345')
