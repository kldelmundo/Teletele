			...
			$.post( "<?=site_url('account/get_start_end_date')?>", {terms:terms} ,function( data ) {
			  ...
			  $("#end").val(data.po_end_date);
				...
				if(principal != null && principal != 0.00 && isNaN(principal) != 1) 
				{
					...
					alert('Invalid amount of principal for min. and max. loanable amount!');
					team2();
				}
			} else {
				//document.getElementById('pterms').value = 0;
				document.getElementById('net_proceeds').value = numberWithCommas(max.toFixed(2));
				document.getElementById('financial_amt').value = max.toFixed(2);
				alert('Invalid amount for principal!');
				team2();
...
<div id="loader" style="display:none"></div>
	...
	function save()
	{
		...
		function(data)
		{
			...
			{
				//sending email
				var jqxhr = $.get( "http://119.93.95.162/For_Evaluation/index.php/Welcome/test_send/"+data.valid_key, function() 
				{
...
<div id="body">
			...
			<table id="table_ledger" style="margin-left:5px;width:90%;background:white;margin-top:-10px;font-size:11px; font-family:tahoma; " class="table table-bordered table-condensed table-hover no-footer" cellspacing="10" border="1">
				...
				<tr>
					...
					<td> <x style="font-weight:bold; color:green; font-size:12px;">Php <?=number_format($row_loan->actual_amount,2);?></x> <small>(<em><strong>Order Date: <?=date('F j, Y',strtotime($row_loan->po_date))?>) </strong></em> </small> </td>
				</tr>
		...
		<div class="slidecontainer">
			...
			<select disabled id="pterms" onchange="team2()">
				<option selected  value='<?=$row_loan->pay_terms?>'><?=round($row_loan->pay_terms)?> Months</option>
			</select>
			</h4>
			
			<table style="width:1000px">
			<tr>
				<td width="200px"><h4>LOAN DETAILS: </h4></td>
			  ...
			  <td><input style="text-align:right; font-weight:bold; color:green" readonly  id="net_proceeds" type="text" value="<?=number_format($row_loan->net_proceeds,2)?>"/></td>
			  </tr>
			   <tr>
			  	<td>GROSS AMOUNT:</td>
			   	<td><input readonly style="text-align:right; font-weight:bold;" id="gross_amount" type="text" value="<?=number_format($row_loan->gross_amount,2)?>"/></td>
			   </tr>
			  <tr>
					
			  <tr>
			  	<td>SEMI MONTHLY:</td>
			   	<td><input readonly style="text-align:right; font-weight:bold;" id="semi" type="text" value="<?=number_format($row_loan->semi_monthly_amor,2)?>"/></td>
			   </tr>
			   
			    <tr>
			  	<td>MONTHLY:</td>
			   	<td><input readonly style="text-align:right; font-weight:bold;" id="monthly" type="text" value="<?=number_format($row_loan->monthly_amor,2)?>"/></td>
			   </tr>
			   
			    <tr>
			  	<td>START DATE:</td>
			   	<td><input readonly style="text-align:right; font-weight:bold;" id="start" type="text" value="<?=date('m/d/Y',strtotime($row_loan->po_start_date))?>"/></td>
			   </tr>
			    <tr>
			  	<td>END DATE:</td>
			   	<td><input readonly style="text-align:right; font-weight:bold;" id="end" type="text" value="<?=date('m/d/Y',strtotime($row_loan->po_end_date))?>"/></td>
			   </tr>
			  <tr>
			  ...
			  <script>
				...
				<h4><button onclick="close_window()">&nbsp;CLOSE&nbsp;</button> </h4>
