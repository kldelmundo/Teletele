...
class M_account extends CI_Model {
    ...
    {
    	$ymd = date("Ymd");
    	$po_number = $this->etbms_db->query("SELECT *
    										  FROM ar_loans_header
    										  WHERE po_number LIKE '%$ymd%'
    										  ORDER BY sales_id DESC
    										  LIMIT 1");

    	if($po_number->num_rows() > 0)
    	{
    		$explode = explode( '-', $po_number->row('po_number') );
			$latest = $explode[2] + 1;
			return "PO-$ymd-$latest";
    	}
    	else
    	{
    		return "PO-$ymd-1";
...
*/
    ...
    function get_dividend()
    {
    	...
    	$sql = "SELECT *
		       	FROM telescoop_web.dividend_2019
				WHERE member_id = $member_id";
    ...
    function get_rbp_benefit($member_id,$date)
	{
		$mem_info = $this->etbms_db->query("SELECT * FROM mem_members WHERE member_id = $member_id")->row();

		#echo $mem_info->dedn_start_dt.' '.$date."<br>";
		$lom = dateDiff($date. '01:00:00', $mem_info->dedn_start_dt. '01:00:00');
		//echo $ctr."->".$lom.': '.$dedn_start_date.'->'.$report->dedn_start_dt.'<br>';
		$explode = explode(' ', $lom);
		#print_r($explode);
		//print_r($ctr.'->'.$explode[1].'<br>');
		if(!empty($lom) AND !empty($explode[1]))
		{
			if($explode[1] == 'years,' || $explode[1] == 'years' || $explode[1] == 'year,' || $explode[1] == 'year')
    		{
    			$years = $explode[0];
    			if($explode[0] > 30)
    			{
    			...
    			$query = $this->tbms_db->query("SELECT * FROM stg_retirement_schedule WHERE yrs = {$explode[0]}");
    			if($query->num_rows() > 0)
    			{
    				$rbp = $query->row('rbp');
    			}
    ...
    function get_loanable_amount_online($member_id)
    {

    	$return_arr = array('loanable_amt'=>0);

        // GET CATEGORY ID PER MEMBER
        $row_m = $this->etbms_db->query("
        SELECT member_id,member_category,emp_level_id,company_id,CONCAT(mem_lname,', ',mem_fname) as mem_name,mem_hired_date,
        ...
        #print_r($row_m);
        #CATEGORY C: PLDT
        if($row_m->company_id == 1){
            $sql_c = "SELECT *
					  FROM  stg_loanable_online
                      WHERE member_type = 3
					  AND emp_levels LIKE '%$row_m->emp_level_id%'
					  AND '$row_m->LOS' BETWEEN LOS_from AND LOS_to
					  LIMIT 1";
        }
        elseif($row_m->member_category == 2)
        {
            $sql_c = "SELECT * FROM  stg_loanable_online WHERE member_type = 1";
        }
        elseif($row_m->member_category == 1)
        {
            $sql_c = "SELECT * FROM  stg_loanable_online WHERE member_type = 2";
        }

        if(isset($sql_c))
		{
        	$cat_id = $this->etbms_db->query($sql_c)->row('cat_id');

	        if(!empty($cat_id)) {

	            $cat_list = $this->etbms_db->query("SELECT * FROM  stg_loanable_online WHERE cat_id = $cat_id");
	            $lastbilling = get_last_billing();
	            $lastbilling_next = switch_next_date($lastbilling);

	            foreach ($cat_list->result() as $row) {

	                $category_where = '';
	                if ($row->member_type == 1) {
	                    $category_where = " AND (company_id = 5)";
	                } elseif ($row->member_type == 2) {
	                    $category_where = ' AND subsidiary = 1 AND member_category = 1';
	                } elseif ($row->member_type == 3) {
	                    $category_where = ' AND company_id = 1 AND member_category = 1';
	                }

	                $level_where = '';
	                if ($row->emp_levels != 0) {
	                    $level_where = " AND emp_level_id IN ($row->emp_levels)";
	                }
	                ...
	                $los_where = '';
	                if ($from != 0 OR $to != 0) {
	                    $los_where = "  AND mem_hired_date NOT IN ('1900-01-01','1970-01-01')
	                                    GROUP BY member_id HAVING LOS BETWEEN $from AND $to
	                                    ORDER BY LOS";
	                }
	                // GETTING MEMBERS
	                $sql = "SELECT company_name,CONCAT(mem_lname,', ',mem_fname) as mem_name,member_id,mem_hired_date,

	                TIMESTAMPDIFF(YEAR, mem_hired_date, CURDATE()) AS LOS,
	                (SELECT beg_bal
	                FROM ar_loans_subs_detail
	                WHERE pay_period = '$lastbilling_next'
	                AND trans_id = 9
	                AND member_id = A.member_id
	                ) SCS,

	                (SELECT zero(SUM(beg_bal))
	                FROM ar_loans_subs_detail
	                WHERE pay_period = '$lastbilling_next'
	                AND trans_id IN (1,2,4,13)
	                AND member_id = A.member_id
	                ) OB,

	                (SELECT zero(MIN(end_bal))
	                FROM ar_loans_subs_detail
	                ...
	                foreach ($query->result() as $row_mem) {
	                    // GET NET STAKE
	                    $net_stake = explode(',', $row->net_stake);
	                    $scs = 0;
	                    $rbp = 0;
	                    foreach ($net_stake as $stake_list) {
	                        switch ($stake_list) {
	                            case 'SCS':
	                                $scs = $row_mem->SCS;
	                                break;
	                            case 'RBP':
	                                $rbp = $this->m_account->get_rbp_benefit($row_mem->member_id, $lastbilling);
	                                break;
	                            case 'Investment':
	                                // AS OF NOW WALA PA
	                                break;
						...
						$ob = $row_mem->OB + $row_mem->FP;



	                    $t_netstake = (($scs + $rbp) - $ob);
	                    if ($row->net_stake_percent > 0) {
	                        $net_stake_percent = $row->net_stake_percent / 100;
	                        $t_netstake = (($scs + $rbp) - $ob) * $net_stake_percent;
						}
						#$loanable_amt = $t_netstake + $row->standard_loan;//delete this if going to uncomment below
						#$loanable_amt = $t_netstake + $row->standard_loan; --> comment lng muna para sa condition by guinmar
						//pag inalis na contion q comment out na dpt yan

//guinmar condition para sa $loanable_amt. pag ayaw na delete lng condition then tangalin comment sa taas.
						if($t_netstake > 0)
						{
							$loanable_amt = $t_netstake + $row->standard_loan;
						}
						else
						{
							$loanable_amt = 0;
						}
//end guinmar condition


// GUINMAR DOINGS START HERE

						if($loanable_amt <= 0 )
						{
							$LOS_check = "SELECT mem_hired_date, TIMESTAMPDIFF(YEAR, mem_hired_date, CURDATE()) AS LOS
											FROM mem_members
											WHERE member_id = $member_id";

							$LOS_length = $this->etbms_db->query($LOS_check)->row('LOS');

							$LOM_check = "SELECT dedn_start_dt, TIMESTAMPDIFF(YEAR, dedn_start_dt, CURDATE()) AS LOM
											FROM mem_members
											WHERE member_id = $member_id";

							$LOM_length = $this->etbms_db->query($LOM_check)->row('LOM');

							$check_comp = $this->etbms_db->query("SELECT company_id FROM mem_members WHERE member_id = '$member_id'")->row('company_id');
							...
							if($check_comp = 1)
							{
								if($LOM_length > 0)
								{
									if($LOS_length > 0 )
									{
											...
											$query_pay_days = $this->etbms_db->query("SELECT pay_period FROM ar_loans_subs_detail
											WHERE pay_period <= '$lastbilling'
											...
											{
												#echo $report->pay_period.'<br>';

												#check def thru payroll deduction
												...
												$sql_check_def = $this->etbms_db->query("SELECT SUM(deferred_amount) as def
																...
																AND member_id = $member_id")->row('def');
...
// GUINMAR DOINGS END HERE

						 //LESS PENDING LOAN APPLICATION
	                    $ol_app = "SELECT SUM(actual_amount) as total
	                    		   FROM ar_loans_online_header
	                    		   WHERE member_id = $row_mem->member_id
								   AND ( (po_order_status = 'pending' AND valid_until > NOW()) OR po_order_status IN ('confirmed','processing') )
								   ";
	                    $ol_query = $this->etbms_db->query($ol_app);

	                    if($ol_query->num_rows() > 0)
	                    {
	                    	$total_avail = $ol_query->row('total');
	                    }

						$loanable_amt -= $total_avail;
						// end

						$max = roundDown($loanable_amt);

	                    if ($t_netstake > $row->net_stake_minimum)
	                    {
	                        ...
	                        $return_arr =  array(
	                            ...
	                            'rbp'           => $rbp,
	                            'ob'            => $ob,
	                            't_netstake'    => $t_netstake,
	                            'standard_loan' => $row->standard_loan,
	                            'loanable_amt'  => number_format($max, 2, '.', '')
	                            );
	                        #---------------------------------------------------------#
	                    }
	                }
